...
class TestingPipeline:
    ...
    Attributes:
        testing_config (TestingConfig): Configuration object containing paths and parameters for the testing process.
    ...
    def predictions_aggregation(self, data: pd.DataFrame, models: List[str], n_folds: int) -> None:
        ...
        if len(models) > 1: # If there are multiple models, calculate the aggregated prediction
            data_copy[PredictionSchema.PREDICTION_AGGREGATED] = (data_copy[models].mean(axis=1) >= self.testing_config.probability_threshold)
    ...
    def evaluate_predictions(self, data: pd.DataFrame, models: List[str]) -> None:
        ...
        with pd.ExcelWriter(self.testing_config.test_kpis_path) as writer:
            results_df.to_excel(writer, sheet_name="Evaluation")
            print(f"KPIs Results saved to {self.testing_config.test_kpis_path}")
    ...
    def run(self):
        ...
        # Load processed data
        df = load_csv_data(data_path=self.testing_config.test_processed_data_filename)

        # Load trained pipelines
        pipelines_dict = load_models(saved_models_path = self.testing_config.saved_models_path)
        ...
        results.to_csv(self.testing_config.test_raw_predictions_path, index=False)
